...
const CreateAccount = () =>{
  ...
  const sendNewUser = async () =>{
    try{
      await axios.post('https://670ed5b73e7151861655eaa3.mockapi.io/Stagiaire',{...formData, id: Math.random()});
  ...
  const passwordVerification = () =>{
    ...
    if (!formData.MotDePasse.match(/[A-Z]/)) validationErrors.push("Password must have an uppercase letter.");
    if (!formData.MotDePasse.match(/[a-z]/)) validationErrors.push("Password must have a lowercase letter.");
    if (!formData.MotDePasse.match(/[0-9]/)) validationErrors.push("Password must have a number.");
    if (!formData.MotDePasse.match(/[^a-zA-Z0-9]/)) validationErrors.push("Password must have a special character.");
    if (formData.MotDePasse.length < 8) validationErrors.push("Password must be at least 8 characters.");
    if (formData.MotDePasse !== formData.confirmMotDePasse) validationErrors.push("Passwords do not match.");
    ...
    if (validationErrors.length > 0) {
      validationErrors.map(el => { alert(el) });
      return false;
    } else{
      return true;
    }
  ...
  return(<>
    <main className="w-screen bg-orange-300 h-screen flex flex-col items-center ">
       ...
       <form onSubmit={handleSubmit} className="w-1/2 flex flex-col items-center p-4 justify-evenly h-full">
          ...
          <div className="w-full flex gap-2">
            ...
            <Input type="password"className="bg-white text-gray-700" name="MotDePasse" value={formData.MotDePasse} onChange={handleChange} />
            </div>
            <div className="w-1/2">
          ...
          <label>Favorite Color:</label>
           <Input type="text" className=" bg-white text-gray-700" name="couleur" value={formData.couleur} onChange={handleChange} />
           </div>
          <div>
